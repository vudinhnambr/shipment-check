...
/**
 * Parses the "Standard Part Name" sheet. Column LAYOUT has changed over time
 * (originally 4 columns: Client, Current Auto MT, Standard, Assembly code; later
 * simplified to 3 columns: Client, Standard Part, Assembly code) - so instead of
 * reading fixed column positions, this looks up each column by matching its header
 * text (row 1), which is stable even if columns get added/removed/reordered.
 * Rows with no Assembly code (e.g. "3.x-103 Pitch Bearing", "6MW") are skipped -
 * they can't be used to build a full bearing set S/N so wouldn't be useful in the
 * part picker.
...
function loadPartsList(rows) {
  ...
  const standardIdx = header.findIndex((h) => h.includes("standard"));
  const codeIdx = header.findIndex((h) => h.includes("assembly") || h.includes("code"));

  const parts = [];
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
    const client = clientIdx >= 0 ? row[clientIdx] : null;
    const standard = standardIdx >= 0 ? row[standardIdx] : null;
